							...
							<table>
								...
								<c:forEach var="lugar" items="${map.value}">
									<td class="fileira-assento"><figure>
										<svg 
										class="assento ${sessao.isDisponivel(lugar) && !carrinho.isSelecionado(lugas) ? 'disponivel' : 'ocupado'}"
										onclick="${sessao.isDisponivel(lugar) && !carrinho.isSelecionado(lugar) ? 'changeCheckbox(this)' : ''}"
										  version="1.0" id="${lugar.id}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
